...
After, you have to download the dataset from [here](https://drive.google.com/file/d/1ruck4EMU-5pkLUTgsAyEgHemEb509gLr/view?usp=drive_link), unzip the archive and place the files in the `data/` directory.  
...
## Running Experiments
To run CoCo experiments, use:
```bash
uv run coco.py
```
To run Ensemble experiments, use:
```bash
uv run ensemble.py
```